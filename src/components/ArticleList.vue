...
function checkAndShowUnreadArticle() {
    ...
    // 获取所有未读公告
    const unreadArticles = articleStore.articles.filter(article =>
        !articleStore.isRead(article.id)
    )
...
async function markAsRead() {
    if (currentArticle.value) {
        ...
        setTimeout(() => {
            // 重新获取所有未读公告
            const remainingUnread = articleStore.articles.filter(article =>
                !articleStore.isRead(article.id)
            )
...
// 监听已读状态变化
watch(() => articleStore.readArticleIds, () => {
}, { deep: true })
</script>

<template>
    <n-modal v-model:show="showModal" preset="card" :title="currentArticle?.title || '系统公告'"
