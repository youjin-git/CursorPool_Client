        ...
        .on_menu_event(move |app, event| {
            match event.id.as_ref() {
                ...
                "switch_machine" => {
                    ...
                    tauri::async_runtime::spawn(async move {
                        match reset_machine_id_only(
                            ids["telemetry.devDeviceId"].clone(),
                            ids["telemetry.macMachineId"].clone(),
                            ids["telemetry.machineId"].clone(),
                            ids["telemetry.sqmId"].clone()
                        ).await {
                            Ok(_) => println!("机器码重置成功"),
                            Err(e) => println!("机器码重置失败: {}", e),
                        }
                    });
                }
                _ => (),
